 ...
 */
const generateTimes = (minTime='00:00', maxTime='24:00', step=15, unavailableTimes=[]) => {
  ...
  for (var time = (minTimeMins - (minTimeMins % 60)); time <= maxTimeMins; time += step) {
    ...
    if (!unavailable)
      times.push(turnInto24Hour(time));
  }
...
const TimePicker = ({ selectedDate, allUnavailableDateTimes, minMaxDayTimes, step }) => {
  ...
  useEffect(() => {
    ...
    if (formattedSelectedDate.valueOf() === today().valueOf() && minMaxDayTimes[7].slice(0,3) === 'now') {
      var addedMins = 0;
