...
const Datetime = ({ minDate, maxDate }) => {
  ...
  if (minDate.slice(0,5) == 'today') {
    ...
    dateToChange = [dateToChange.getFullYear(), numberToTwoChar(dateToChange.getMonth() + 1), numberToTwoChar(dateToChange.getDate())].join('-');;
    minDate = dateToChange;
  } else if (minDate instanceof Date) {
    var unformattedDate = [minDate.getFullYear(), numberToTwoChar(minDate.getMonth() + 1), numberToTwoChar(minDate.getDate())].join('-');
  ...
  if (maxDate == 'today') {
    ...
    var unformattedDate = [maxDate.getFullYear(), numberToTwoChar(maxDate.getMonth() + 1), numberToTwoChar(maxDate.getDate())].join('-');
  ...
  function selectDate(date) {
    if (typeof (date) == 'string' && date != selectedDate) {
      if (date.slice(0, -3) === monthYear) {
        ...
        const formattedCurrentMonth = new Date(monthYear.slice(0,4), monthYear.slice(5,7))
        const formattedNewMonth = new Date(newMonthYear.slice(0,4), newMonthYear.slice(5,7))
